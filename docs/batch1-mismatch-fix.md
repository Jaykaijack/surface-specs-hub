...
| 4 | `laptop-7-biz-intel.touchAndPenProtocol` / `penCompat` | 不支持触控笔 | microsoft.com Surface Laptop for Business 规格（与 Laptop 7/8 官方一致） |
...
推送：累积更新 `docs/p0-surface-data.patch.b64`（相对 BASE `3374f26`），由 Actions `p0-apply-surface-data-patch` 还原 BASE 并 patch → 提交 `js/surface-data.js`。
